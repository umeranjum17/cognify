/* API Security End-to-End Test for Express Backend
...
if (!FIREBASE_API_KEY || !TEST_EMAIL || !TEST_PASSWORD) {
  console.error('Missing env: FIREBASE_API_KEY, TEST_EMAIL, TEST_PASSWORD');
  process.exit(1);
}
...
async function run() {
  ...
  console.log('🧪 Testing Express Backend API Security\n');
  console.log('=== Health Check ===');
  try {
    const r = await request('GET', '/health');
    assert(r.status === 200, `Health check expected 200, got ${r.status}`);
    assert(r.isJson, 'Health check expected JSON');
    pass('GET /health (200)');
  } catch (e) {
    fail(`GET /health: ${e.message}`);
  }

  console.log('\n=== Unauthenticated checks (expect 401) ===');
  const unauthTests = [
  ...
  try {
    ...
    if (RC_WEBHOOK_SECRET) {
      ...
      console.log('⏭️  SKIP /api/rc/webhook with valid secret: RC_WEBHOOK_SECRET not set');
    }
  } catch (e) {
  ...
  try {
    ...
    // 501 is expected since mermaid is not implemented yet
    assert([200, 501].includes(r.status), `/api/mermaid/generate expected 200/501, got ${r.status}`);
    pass(`POST /api/mermaid/generate (${r.status}${r.status === 501 ? ' - not implemented yet' : ''})`);
  } catch (e) {

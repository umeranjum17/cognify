...
/// Executor Agent - Executes tools based on plans from Planner Agent
class ExecutorAgent {
  ...
  Future<List<ToolResult>> executeTools(
    ...
    if (!_initialized) {
      throw Exception('Executor Agent not initialized');
    }
  ...
  Future<void> initialize() async {
    try {
      print('⚡ Initializing Executor Agent (tool execution engine with isolate pool)');
      ...
      _initialized = true;
      print('✅ Executor Agent initialized with ${_toolsManager.allTools.length} tools and isolate pool');
    } catch (error) {
      print('❌ Executor Agent initialization failed: $error');
      rethrow;
    }
  }
  ...
  /// REMOVED: Enhanced source and image extraction - now using ToolResultProcessor
